...
- ~~Home / End on multi-line input (whole-input bounds vs. current-visual-line).~~ **Pinned in 1.E:** **whole-input bounds**. The `home` / `end` cases call `setCursor(0)` / `setCursor(_input.length)` directly — the visual row of the cursor is irrelevant. This differs from common shell behavior (bash/readline), which moves only within the current visual line.
...
- ~~Multi-line paste containing a line with incomplete shell syntax — does it continue across the newline or submit as-is?~~ **Pinned in 1.G:** it **continues** across the newline. `handleTermData` collapses `[\r\n]+` → `\r` and feeds chars one by one, and the Enter case treats `isIncompleteInput` as "insert literal `\n`", so the paste accumulates into a single multi-line input until a `\r` lands on a complete state. (Aside: when a pasted `\r` does land on a complete state, `handleReadComplete` flips `_active=false` and the rest of the paste is silently dropped — see BUG-006.)
