...
const LIB_PACKAGE_DIR = path.resolve(__dirname, "./packages/protostar")
const LIB_SHIMS_DIR = path.resolve(LIB_PACKAGE_DIR, "src/shims")

/**
 * Root playground dev/app config. The library now lives in
 * `packages/protostar/`; this config consumes it via the workspace symlink
 ...
 *
 * Shim aliases come along for the ride: in dev (and app build) the lib's
 * `src/library.ts` is bundled directly through Vite's resolver, so pi-tui's
 * Node-only transitive imports (`node:module`, `node:perf_hooks`,
 * `child_process`, `fs`, `os`, `path`, `events`) need to resolve to the
 * same shims the lib build uses. Phase 3.D bakes these shims into the
 * published lib bundle and removes this duplication on the consumer side;
 * for now the playground still bundles lib source through Vite, so the
 * aliases stay.
 */
