...
test.fixme("Ctrl+C with a typed partial prints ^C, clears input, and reprompts (BUG-015)", async ({ page }) => {
...
test.fixme("Ctrl+C on empty input prints ^C and reprompts (BUG-015)", async ({ page }) => {
...
test.fixme("Ctrl+C during multi-line continuation exits cleanly to a fresh prompt (BUG-015)", async ({ page }) => {
...
test.fixme("Cancelled input is not added to history (BUG-015)", async ({ page }) => {
